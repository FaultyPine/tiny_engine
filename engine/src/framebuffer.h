...
struct Framebuffer {
    ...
    TAPI Texture GetTexture() const { return Texture(texture); }
...
struct ShadowMap {
    ...
    TAPI void ReceiveShadows(Shader& shader, const Light& light) const;
    // makes passed in model cast shadows (just renders the model into the depth texture)
    TAPI void RenderShadowCaster(const Light& light, Model& model, const Transform& tf) const;
